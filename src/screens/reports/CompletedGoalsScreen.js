...
export default function CompletedGoalsScreen({ navigation }) {
  const { goals } = useGoals();
  const completedGoals = goals.filter((g) => g.subgoals?.every((s) => s.isDone)).length;

  const { width, height } = useWindowDimensions();
  const L = Math.max(width, height);
  const digits = String(completedGoals).length;
